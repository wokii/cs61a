...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    rem, quot = score % 10, score // 10
    return 2 + abs(rem-quot)
...
def is_swap(score0, score1):
    ...
    if score0 <= 1 or score1 <= 1:
        return False
    elif not score0 % score1 or not score1 % score0:
        return True
    else:
        return False
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    #score0 += take_turn(strategy(score0,score1),score1,dice)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if gain > previous_high:
            #为什么不能在这里写 previous_high = gain
            """ 在这里有解释为什么不能这么写：https://www.youtube.com/watch?v=hWZWjwoKVCk&list=PL6BsET-8jgYXF2TzAQRQeaZwhp-9DfE1r&index=7

                我的理解是python中的elevation把之后要写的previous_high提升到了最前面，而
                elevation只提升name 不提升实际赋的值，所以此时previous_high还是未赋值状态
                而因为当前frame已经有previous_high，所以不采用父系方程中previous_high的值
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    while(i <= 10):
        current = averaged_roll(i, dice)
        #print ("current:", current)
        if current > high:
            #print("current", current, "high", high)
            high = current
            result = i
        i += 1
    return result
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    fb_score = score + free_bacon(opponent_score) #score after choosing freebacon
    if is_swap(fb_score, opponent_score) and fb_score < opponent_score:
        return 0
    elif fb_score - score >= margin and not is_swap(fb_score, opponent_score):
        return 0
    else:
        return num_rolls
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if GOAL_SCORE - score <= free_bacon(opponent_score):
        return 0
    elif GOAL_SCORE - score <= 2:
        return 1
